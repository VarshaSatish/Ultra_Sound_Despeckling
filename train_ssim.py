...
def pbar_desc(label, epoch, total_epochs, loss_val, losses):
    return f'{label}:{epoch:04d}/{total_epochs} | {loss_val: .3f} | ssim: {losses["ssim"]}'

def train(Unet,trn_dl,epoch,epochs,SSIM_loss,opt,train_losses):
    Unet.train()
    t_pbar = tqdm(trn_dl, desc=pbar_desc('train',epoch,epochs,0.0,{'ssim':0.0}))
    for lr_imgs, hr_imgs in t_pbar:
        ...
        #print(hr_imgs.shape)        
        #print(sr_imgs.shape)
        ssim_loss = SSIM_loss(hr_imgs,sr_imgs)
        ssim_display = ssim_loss.detach().cpu().item()

        loss = SSIM_LOSS_WEIGHT * ssim_loss
        opt.zero_grad()
        loss.backward()
        opt.step()

        t_pbar.set_description(pbar_desc('train',epoch,EPOCHS,loss.item(), {'ssim': round(ssim_display,3)}))

        train_losses.update(ssim = ssim_loss.item())


def evaluate(Unet,val_dl,epoch,epochs,SSIM_loss,val_losses,best_val_loss):
    Unet.eval()
    v_pbar = tqdm(val_dl,desc=pbar_desc('valid', epoch , epochs, 0.0, {'ssim':0.0}))
    with torch.no_grad():
        e = 0.0
        for lr_imgs,hr_imgs in v_pbar:
            ...
            val_losses.update(ssim = ssim_loss.item())
            v_pbar.set_description(pbar_desc('valid',epoch, EPOCHS , loss.item(), {'ssim':round(ssim_display,3)}))
            if e == 10:
    ...
    avg_val_loss = avg_val_losses['ssim']
...
def main():
    ...
    for epoch in range(start_epoch, EPOCHS+1):
        ## training loop
        train(Unet, trn_dl,epoch,EPOCHS,SSIM_loss,opt,train_losses)

        ## validation loop
        best_val_loss = evaluate(Unet, val_dl, epoch, EPOCHS, SSIM_loss, val_losses, best_val_loss)
        sched.step()
